...
export const en: Catalog = {
  ...
  impl_desconhecida: () => 'origin not declared',
  ...
  v_tubo: () => '3D tube',
  v_tubo_dica: () => 'Drag to rotate. The seam does not show as a scar because the embedding is helical: every lateral bond has the same length, that one included. The seam is a lattice discontinuity — an α–α contact where the rest has α–β — not a geometric one.',
  v_tubo_fechado: () => 'With the tube closed longitudinally, the bonds joining the two ends cut across the cylinder in the drawing: the graph is periodic and the cylinder cannot be.',
  v_tubo_metrico: () => 'metric scale',
  v_tubo_escala: (p) => `Axis compressed ×${p['fator']}: distances along the tube are NOT on the same scale as the lateral ones. A microtubule is far too long to fit at true scale — at ×1 the scale is honest and the tube becomes a hair.`,
